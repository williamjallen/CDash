      ...
      <table class="tabb" style="width: 100%;">
        ...
        <tbody>
          ...
          <tr v-for="(token, hash, index) in cdash.tokens" :class="{'treven': index % 2 === 0, 'trodd': index % 2 !== 0}">
            <td align="center">{{ token.owner_firstname }}&nbsp;{{ token.owner_lastname }}</td>
            <td align="center">{{ token.description }}</td>
            <td align="center"><a :href="$baseURL + '/index.php?project=' + token.projectname">{{ token.projectname }}</a></td>
            <td align="center">{{ token.expires }}</td>
            <td align="center">
